...
| Gemma4 26B-A4B | P3-A FormClassifier + P3-B StructuredExtractor | google/gemma-4-26b-a4b-it | `models/gemma4/gemma4-mil-finetuned/` | ❌ 미시작 (Phase 2) |
...
## 5. v1 → v2 코드 이관 계획 (고도화 반영)

### 5-1. 이관 대상 분류

| v1 소스 파일 | v2 대상 | 이관 방식 |
|-------------|---------|----------|
...
| `src/layer3_correction/t9_confidence_scorer.py` | `src/postprocess/validator.py` | **대폭 수정** — 처리 경로별 임계값 분리, 길이 편향 보정 |
...
| v1 소스 파일 | 사유 |
|-------------|------|
| `t3_form_classifier.py` | P3-A FormClassifier로 대체 |
| `t4_handwriting_recognizer.py` | P3-B StructuredExtractor로 대체 |
| `t5_structure_recognizer.py` | P3-B StructuredExtractor로 대체 |
| `t6_domain_code_recognizer.py` | P3-B instruction에 포함 |
| `t7_language_model_corrector.py` | P3-B VLM 문맥 교정으로 대체 |
| `t8_data_validator.py` | P4 룰 검증에 병합 |
| `t10_schema_mapper.py` | P3-B guided_json으로 대체 |

### 5-3. 신규 구현 (v1에 없는 것)

| v2 소스 파일 | 내용 |
|-------------|------|
| `src/preprocess/layout_postprocessor.py` | P2.5-A — 미소/중복 박스 제거, 블록 병합 |
| `src/vlm/form_classifier.py` | P3-A — 전체 페이지 저해상도 서식 분류 |
| `src/vlm/instruction_router.py` | P2.5-B — form_type 반영 도메인 instruction (instruction_builder.py 대체) |
| `src/vlm/resolution_router.py` | P2.5-C — 영역별 pixel_budget 할당 + 배치 그룹화 |
| `src/vlm/structured_extractor.py` | P3-B — 배치 병렬 VLM 추출 (gemma4_engine.py 분리) |
...
2순위: preprocess/ (P1 + P2 + P2.5-A LayoutPostProcessor)
